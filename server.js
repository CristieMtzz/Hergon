...
const JWT_SECRET = 'hergon-secret-key-2026';
...
function crearNotificacion(tipo, mensaje, sucursalOrigenId, datos = null) {
  const stmt = db.prepare('INSERT INTO notificaciones (tipo, mensaje, sucursal_origen_id, datos) VALUES (?,?,?,?)');
  const result = stmt.run(tipo, mensaje, sucursalOrigenId, datos ? JSON.stringify(datos) : null);
  const notif = db.prepare('SELECT n.*, s.nombre as sucursal_nombre FROM notificaciones n LEFT JOIN sucursales s ON n.sucursal_origen_id=s.id WHERE n.id=?').get(result.lastInsertRowid);
  io.emit('notificacion', notif);
  return notif;
}

// Helper: generar número de ticket
function generarTicket(sucursalId) {
  const fecha = new Date();
  const prefix = `T${sucursalId}-${fecha.getFullYear()}${String(fecha.getMonth()+1).padStart(2,'0')}${String(fecha.getDate()).padStart(2,'0')}`;
  const count = db.prepare("SELECT COUNT(*) as c FROM ventas WHERE ticket_numero LIKE ?").get(prefix + '%');
...
app.post('/api/auth/login', (req, res) => {
  ...
  const user = db.prepare('SELECT u.*, s.nombre as sucursal_nombre FROM usuarios u LEFT JOIN sucursales s ON u.sucursal_id=s.id WHERE u.usuario=? AND u.activo=1').get(usuario);
...
app.post('/api/auth/verify-pin', authMiddleware, (req, res) => {
  const { pin } = req.body;
  const admin = db.prepare('SELECT * FROM usuarios WHERE rol="administrador" AND activo=1').get();
...
app.get('/api/sucursales', authMiddleware, (req, res) => {
  res.json(db.prepare('SELECT * FROM sucursales').all());
});
...
app.get('/api/productos', authMiddleware, (req, res) => {
  ...
  for (const p of productos) {
    p.inventario = db.prepare('SELECT i.*, s.nombre as sucursal_nombre FROM inventario i JOIN sucursales s ON i.sucursal_id=s.id WHERE i.producto_id=?').all(p.id);
    if (sucursal_id) {
...
app.post('/api/productos', authMiddleware, adminOnly, (req, res) => {
  ...
  try {
    const result = db.prepare(`INSERT INTO productos (clave, nombre, descripcion, categoria_id, precio_menudeo, precio_medio_mayoreo, precio_mayoreo, cantidad_medio_mayoreo, cantidad_mayoreo, stock_minimo) VALUES (?,?,?,?,?,?,?,?,?,?)`)
    ...
    // Init inventory
    const sucursales = db.prepare('SELECT id FROM sucursales').all();
    for (const s of sucursales) {
      const stock = (stocks && stocks[s.id]) || 0;
      db.prepare('INSERT INTO inventario (producto_id, sucursal_id, stock) VALUES (?,?,?)').run(prodId, s.id, stock);
    }
    crearNotificacion('producto_nuevo', `Nuevo producto registrado: ${clave} - ${nombre}`, null);
...
app.put('/api/productos/:id', authMiddleware, adminOnly, (req, res) => {
  ...
  db.prepare(`UPDATE productos SET nombre=?, descripcion=?, categoria_id=?, precio_menudeo=?, precio_medio_mayoreo=?, precio_mayoreo=?, cantidad_medio_mayoreo=?, cantidad_mayoreo=?, stock_minimo=? WHERE id=?`)
...
app.post('/api/inventario/entrada', authMiddleware, (req, res) => {
  const { producto_id, sucursal_id, cantidad } = req.body;
  db.prepare('UPDATE inventario SET stock = stock + ?, updated_at = CURRENT_TIMESTAMP WHERE producto_id=? AND sucursal_id=?').run(cantidad, producto_id, sucursal_id);
  db.prepare('INSERT INTO movimientos_inventario (producto_id, sucursal_id, tipo, cantidad, usuario_id) VALUES (?,?,?,?,?)').run(producto_id, sucursal_id, 'entrada', cantidad, req.user.id);
  const prod = db.prepare('SELECT clave, nombre FROM productos WHERE id=?').get(producto_id);
  const suc = db.prepare('SELECT nombre FROM sucursales WHERE id=?').get(sucursal_id);
  crearNotificacion('entrada_mercancia', `Entrada de ${cantidad} unidades de ${prod.clave} en ${suc.nombre}`, sucursal_id);
  res.json({ message: 'Entrada registrada' });
});

app.post('/api/inventario/traspaso', authMiddleware, adminOnly, (req, res) => {
  const { producto_id, sucursal_origen_id, sucursal_destino_id, cantidad } = req.body;
  const inv = db.prepare('SELECT stock FROM inventario WHERE producto_id=? AND sucursal_id=?').get(producto_id, sucursal_origen_id);
  if (!inv || inv.stock < cantidad) return res.status(400).json({ error: 'Stock insuficiente en sucursal de origen' });

  db.prepare('UPDATE inventario SET stock = stock - ?, updated_at = CURRENT_TIMESTAMP WHERE producto_id=? AND sucursal_id=?').run(cantidad, producto_id, sucursal_origen_id);
  db.prepare('UPDATE inventario SET stock = stock + ?, updated_at = CURRENT_TIMESTAMP WHERE producto_id=? AND sucursal_id=?').run(cantidad, producto_id, sucursal_destino_id);
  db.prepare('INSERT INTO movimientos_inventario (producto_id, sucursal_id, tipo, cantidad, usuario_id) VALUES (?,?,?,?,?)').run(producto_id, sucursal_origen_id, 'traspaso_salida', cantidad, req.user.id);
  db.prepare('INSERT INTO movimientos_inventario (producto_id, sucursal_id, tipo, cantidad, usuario_id) VALUES (?,?,?,?,?)').run(producto_id, sucursal_destino_id, 'traspaso_entrada', cantidad, req.user.id);

  const prod = db.prepare('SELECT clave, nombre FROM productos WHERE id=?').get(producto_id);
  const sucOr = db.prepare('SELECT nombre FROM sucursales WHERE id=?').get(sucursal_origen_id);
  const sucDe = db.prepare('SELECT nombre FROM sucursales WHERE id=?').get(sucursal_destino_id);
  crearNotificacion('traspaso', `Traspaso de ${cantidad} uds de ${prod.clave} de ${sucOr.nombre} a ${sucDe.nombre}`, sucursal_origen_id);
  res.json({ message: 'Traspaso realizado' });
});

// ===================== PROMOCIONES =====================
app.get('/api/promociones', authMiddleware, (req, res) => {
  ...
  if (activas === '1') {
    sql += ` WHERE pr.activo=1 AND DATE(pr.fecha_inicio) <= DATE('now') AND DATE(pr.fecha_fin) >= DATE('now')`;
  }
  sql += ` ORDER BY pr.fecha_fin DESC`;
  res.json(db.prepare(sql).all());
});

app.post('/api/promociones', authMiddleware, adminOnly, (req, res) => {
  const { nombre, tipo, valor, aplica_a, producto_id, categoria_id, fecha_inicio, fecha_fin } = req.body;
  const result = db.prepare('INSERT INTO promociones (nombre, tipo, valor, aplica_a, producto_id, categoria_id, fecha_inicio, fecha_fin) VALUES (?,?,?,?,?,?,?,?)')
    .run(nombre, tipo, valor, aplica_a, producto_id || null, categoria_id || null, fecha_inicio, fecha_fin);
  crearNotificacion('promocion', `Nueva promoción: ${nombre}`, null);
  res.json({ id: result.lastInsertRowid, message: 'Promoción creada' });
});

app.put('/api/promociones/:id/toggle', authMiddleware, adminOnly, (req, res) => {
  const promo = db.prepare('SELECT activo FROM promociones WHERE id=?').get(req.params.id);
  db.prepare('UPDATE promociones SET activo=? WHERE id=?').run(promo.activo ? 0 : 1, req.params.id);
...
app.post('/api/clientes', authMiddleware, (req, res) => {
  ...
  const result = db.prepare('INSERT INTO clientes (nombre, telefono, email, membresia_id) VALUES (?,?,?,?)').run(nombre, telefono, email || null, membresia_id || null);
  res.json({ id: result.lastInsertRowid, message: 'Cliente registrado' });
});

app.put('/api/clientes/:id', authMiddleware, (req, res) => {
  const { nombre, telefono, email, membresia_id } = req.body;
  db.prepare('UPDATE clientes SET nombre=?, telefono=?, email=?, membresia_id=? WHERE id=?').run(nombre, telefono, email, membresia_id || null, req.params.id);
  res.json({ message: 'Cliente actualizado' });
});

app.get('/api/clientes/:id/historial', authMiddleware, (req, res) => {
  const ventas = db.prepare(`SELECT v.*, s.nombre as sucursal_nombre, u.nombre as cajero_nombre FROM ventas v
...
app.get('/api/tipos-membresia', authMiddleware, (req, res) => {
  res.json(db.prepare('SELECT * FROM tipos_membresia WHERE activo=1').all());
});
...
app.post('/api/ventas', authMiddleware, (req, res) => {
  ...
  const ticket_numero = generarTicket(sucursal_id);

  // Get active promotions
  const promos = db.prepare(`SELECT * FROM promociones WHERE activo=1 AND DATE(fecha_inicio) <= DATE('now') AND DATE(fecha_fin) >= DATE('now')`).all();

  // Get client membership discount
  let descuentoMembresia = 0;
  if (cliente_id) {
    const cli = db.prepare('SELECT tm.descuento FROM clientes cl LEFT JOIN tipos_membresia tm ON cl.membresia_id=tm.id WHERE cl.id=?').get(cliente_id);
  ...
  for (const item of items) {
    ...
    const inv = db.prepare('SELECT stock FROM inventario WHERE producto_id=? AND sucursal_id=?').get(item.producto_id, sucursal_id);
  ...
  // Insert sale
  const ventaResult = db.prepare(`INSERT INTO ventas (ticket_numero, sucursal_id, cajero_id, cliente_id, subtotal, descuento_membresia, descuento_promocion, total, tipo_pago, autorizado_por) VALUES (?,?,?,?,?,?,?,?,?,?)`)
  ...
  for (const d of detalles) {
    db.prepare('INSERT INTO venta_detalle (venta_id, producto_id, cantidad, precio_unitario, tipo_precio, descuento_promo, subtotal) VALUES (?,?,?,?,?,?,?)').run(ventaId, d.producto_id, d.cantidad, d.precio_unitario, d.tipo_precio, d.descuento_promo, d.subtotal);
    db.prepare('UPDATE inventario SET stock = stock - ?, updated_at = CURRENT_TIMESTAMP WHERE producto_id=? AND sucursal_id=?').run(d.cantidad, d.producto_id, sucursal_id);
    db.prepare('INSERT INTO movimientos_inventario (producto_id, sucursal_id, tipo, cantidad, referencia, usuario_id) VALUES (?,?,?,?,?,?)').run(d.producto_id, sucursal_id, 'venta', d.cantidad, ticket_numero, req.user.id);

    // Check low stock alert
    const invAfter = db.prepare('SELECT i.stock, p.stock_minimo, p.clave, p.nombre FROM inventario i JOIN productos p ON i.producto_id=p.id WHERE i.producto_id=? AND i.sucursal_id=?').get(d.producto_id, sucursal_id);
    if (invAfter && invAfter.stock <= invAfter.stock_minimo) {
      const sucNombre = db.prepare('SELECT nombre FROM sucursales WHERE id=?').get(sucursal_id);
      const tipoAlerta = invAfter.stock === 0 ? 'stock_cero' : 'stock_bajo';
      crearNotificacion(tipoAlerta, `${tipoAlerta === 'stock_cero' ? '🔴 SIN STOCK' : '⚠️ Stock bajo'}: ${invAfter.clave} - ${invAfter.nombre} (${invAfter.stock} uds) en ${sucNombre.nombre}`, sucursal_id);
    }
  }

  const sucNombre = db.prepare('SELECT nombre FROM sucursales WHERE id=?').get(sucursal_id);
  crearNotificacion('venta', `Venta ${ticket_numero} por $${total.toFixed(2)} (${tipo_pago}) en ${sucNombre.nombre}`, sucursal_id);

  // Return full ticket data
  const venta = db.prepare(`SELECT v.*, s.nombre as sucursal_nombre, s.direccion as sucursal_direccion, u.nombre as cajero_nombre, cl.nombre as cliente_nombre
  ...
  venta.detalles = db.prepare(`SELECT vd.*, p.clave, p.nombre as producto_nombre FROM venta_detalle vd JOIN productos p ON vd.producto_id=p.id WHERE vd.venta_id=?`).all(ventaId);
...
app.get('/api/ventas/:id', authMiddleware, (req, res) => {
  const venta = db.prepare(`SELECT v.*, s.nombre as sucursal_nombre, s.direccion as sucursal_direccion, s.ciudad as sucursal_ciudad, u.nombre as cajero_nombre, cl.nombre as cliente_nombre
  ...
  venta.detalles = db.prepare(`SELECT vd.*, p.clave, p.nombre as producto_nombre FROM venta_detalle vd JOIN productos p ON vd.producto_id=p.id WHERE vd.venta_id=?`).all(venta.id);
  res.json(venta);
});
...
app.post('/api/devoluciones', authMiddleware, (req, res) => {
  ...
  let totalDev = 0;
  const devResult = db.prepare('INSERT INTO devoluciones (venta_id, cajero_id, autorizado_por, sucursal_id, motivo) VALUES (?,?,?,?,?)')
    .run(venta_id, req.user.id, admin.id, venta.sucursal_id, motivo || '');
  const devId = devResult.lastInsertRowid;

  for (const item of items) {
    const detalle = db.prepare('SELECT * FROM venta_detalle WHERE venta_id=? AND producto_id=?').get(venta_id, item.producto_id);
    if (!detalle) continue;
    const subtotal = detalle.precio_unitario * item.cantidad;
    totalDev += subtotal;
    db.prepare('INSERT INTO devolucion_detalle (devolucion_id, producto_id, cantidad, precio_unitario, subtotal) VALUES (?,?,?,?,?)').run(devId, item.producto_id, item.cantidad, detalle.precio_unitario, subtotal);
    db.prepare('UPDATE inventario SET stock = stock + ?, updated_at = CURRENT_TIMESTAMP WHERE producto_id=? AND sucursal_id=?').run(item.cantidad, item.producto_id, venta.sucursal_id);
    db.prepare('INSERT INTO movimientos_inventario (producto_id, sucursal_id, tipo, cantidad, referencia, usuario_id) VALUES (?,?,?,?,?,?)').run(item.producto_id, venta.sucursal_id, 'devolucion', item.cantidad, venta.ticket_numero, req.user.id);
  }

  db.prepare('UPDATE devoluciones SET total=? WHERE id=?').run(totalDev, devId);
  db.prepare("UPDATE ventas SET estado='parcial_devuelta' WHERE id=?").run(venta_id);

  const sucNombre = db.prepare('SELECT nombre FROM sucursales WHERE id=?').get(venta.sucursal_id);
  crearNotificacion('devolucion', `Devolución de $${totalDev.toFixed(2)} del ticket ${venta.ticket_numero} en ${sucNombre.nombre}`, venta.sucursal_id);
...
app.get('/api/cortes/resumen', authMiddleware, (req, res) => {
  const cajeroId = req.user.id;
  const sucursalId = req.user.sucursal_id;
  // Get last corte time
  const lastCorte = db.prepare('SELECT created_at FROM cortes_caja WHERE cajero_id=? ORDER BY created_at DESC LIMIT 1').get(cajeroId);
  const since = lastCorte ? lastCorte.created_at : '2000-01-01';

  const ventas = db.prepare(`SELECT * FROM ventas WHERE cajero_id=? AND sucursal_id=? AND created_at > ? AND estado != 'devuelta'`).all(cajeroId, sucursalId, since);
  const devoluciones = db.prepare(`SELECT d.* FROM devoluciones d WHERE d.cajero_id=? AND d.sucursal_id=? AND d.created_at > ?`).all(cajeroId, sucursalId, since);
...
app.post('/api/cortes', authMiddleware, (req, res) => {
  ...
  const result = db.prepare(`INSERT INTO cortes_caja (cajero_id, sucursal_id, total_ventas, total_efectivo, total_transferencia, total_tarjeta, total_devoluciones, monto_esperado, monto_real, diferencia, num_ventas) VALUES (?,?,?,?,?,?,?,?,?,?,?)`)
  ...
  const sucNombre = db.prepare('SELECT nombre FROM sucursales WHERE id=?').get(sucursalId);
  crearNotificacion('corte_caja', `Corte de caja de ${req.user.nombre} en ${sucNombre.nombre}: $${totalVentas.toFixed(2)} (dif: $${diferencia.toFixed(2)})`, sucursalId);

  if (Math.abs(diferencia) > 100) {
    crearNotificacion('alerta_corte', `⚠️ Diferencia significativa en corte de ${req.user.nombre}: $${diferencia.toFixed(2)} en ${sucNombre.nombre}`, sucursalId);
  }
...
app.get('/api/usuarios', authMiddleware, adminOnly, (req, res) => {
  res.json(db.prepare(`SELECT u.id, u.nombre, u.usuario, u.rol, u.sucursal_id, u.activo, u.email, u.created_at, s.nombre as sucursal_nombre FROM usuarios u LEFT JOIN sucursales s ON u.sucursal_id=s.id ORDER BY u.nombre`).all());
});
...
app.post('/api/usuarios', authMiddleware, adminOnly, (req, res) => {
  ...
  try {
    ...
    const result = db.prepare('INSERT INTO usuarios (nombre, usuario, password, rol, sucursal_id, email, pin) VALUES (?,?,?,?,?,?,?)')
...
app.get('/api/notificaciones', authMiddleware, (req, res) => {
  const notifs = db.prepare(`SELECT n.*, s.nombre as sucursal_nombre,
...
app.post('/api/notificaciones/:id/leer', authMiddleware, (req, res) => {
  try {
    db.prepare('INSERT OR IGNORE INTO notificaciones_leidas (notificacion_id, usuario_id) VALUES (?,?)').run(req.params.id, req.user.id);
  } catch (e) { /* already read */ }
  res.json({ ok: true });
});

app.get('/api/notificaciones/no-leidas', authMiddleware, (req, res) => {
  const count = db.prepare(`SELECT COUNT(*) as c FROM notificaciones n WHERE n.id NOT IN (SELECT notificacion_id FROM notificaciones_leidas WHERE usuario_id=?)`).get(req.user.id);
...
app.get('/api/reportes/ventas', authMiddleware, (req, res) => {
  ...
  const resumen = db.prepare(`SELECT COUNT(*) as total_ventas, COALESCE(SUM(v.total),0) as monto_total,
  ...
  let detalle;
  if (agrupar === 'sucursal') {
    detalle = db.prepare(`SELECT s.nombre as grupo, COUNT(*) as ventas, SUM(v.total) as monto FROM ventas v JOIN sucursales s ON v.sucursal_id=s.id WHERE ${conditions} GROUP BY v.sucursal_id`).all(...params);
  } else if (agrupar === 'cajero') {
    detalle = db.prepare(`SELECT u.nombre as grupo, COUNT(*) as ventas, SUM(v.total) as monto FROM ventas v JOIN usuarios u ON v.cajero_id=u.id WHERE ${conditions} GROUP BY v.cajero_id`).all(...params);
  } else if (agrupar === 'dia') {
    detalle = db.prepare(`SELECT DATE(v.created_at) as grupo, COUNT(*) as ventas, SUM(v.total) as monto FROM ventas v WHERE ${conditions} GROUP BY DATE(v.created_at) ORDER BY grupo`).all(...params);
  } else {
    detalle = db.prepare(`SELECT v.ticket_numero, v.total, v.tipo_pago, v.created_at, s.nombre as sucursal, u.nombre as cajero FROM ventas v JOIN sucursales s ON v.sucursal_id=s.id JOIN usuarios u ON v.cajero_id=u.id WHERE ${conditions} ORDER BY v.created_at DESC LIMIT 200`).all(...params);
  }

  res.json({ resumen, detalle });
});

app.get('/api/reportes/productos-top', authMiddleware, (req, res) => {
  ...
  const top = db.prepare(`SELECT p.clave, p.nombre, SUM(vd.cantidad) as total_vendido, SUM(vd.subtotal) as total_monto
    FROM venta_detalle vd JOIN ventas v ON vd.venta_id=v.id JOIN productos p ON vd.producto_id=p.id
    WHERE ${conditions} GROUP BY vd.producto_id ORDER BY total_vendido DESC LIMIT ?`).all(...params, parseInt(lim) || 20);
  res.json(top);
});
...
app.get('/api/dashboard', authMiddleware, (req, res) => {
  ...
  const ventasHoy = db.prepare(`SELECT COUNT(*) as num, COALESCE(SUM(total),0) as monto FROM ventas WHERE DATE(created_at) = ?`).get(hoy);
  const alertasStock = db.prepare(`SELECT COUNT(*) as c FROM inventario i JOIN productos p ON i.producto_id=p.id WHERE i.stock <= p.stock_minimo AND p.activo=1`).get();
  const totalProductos = db.prepare(`SELECT COUNT(*) as c FROM productos WHERE activo=1`).get();
  const ventasPorSucursal = db.prepare(`SELECT s.nombre, COUNT(v.id) as ventas, COALESCE(SUM(v.total),0) as monto FROM sucursales s LEFT JOIN ventas v ON s.id=v.sucursal_id AND DATE(v.created_at)=? GROUP BY s.id`).all(hoy);
