...
const DB_PATH = path.join(__dirname, 'hergon.db');

// Wrapper to mimic better-sqlite3 API over sql.js
class DBWrapper {
...
async function initDatabase() {
  ...
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS clientes (
      ...
      created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
      FOREIGN KEY (membresia_id) REFERENCES tipos_membresia(id)
    );
    ...
    CREATE TABLE IF NOT EXISTS notificaciones_leidas (
      ...
      leida_at DATETIME DEFAULT CURRENT_TIMESTAMP,
  ...
  if (countSuc.c === 0) {
    ...
    const insertSuc = (n, d, c, m) => db.prepare('INSERT INTO sucursales (nombre, direccion, ciudad, es_matriz) VALUES (?,?,?,?)').run(n, d, c, m);
    insertSuc('HERGON Matriz Coatepec', 'Centro, Coatepec', 'Coatepec', 1);
    insertSuc('HERGON Xalapa Centro', 'Centro, Xalapa', 'Xalapa', 0);
    insertSuc('HERGON Xalapa Sur', 'Zona Sur, Xalapa', 'Xalapa', 0);
    insertSuc('HERGON Xalapa Norte', 'Zona Norte, Xalapa', 'Xalapa', 0);
    ...
    for (const [n, d] of cats) db.prepare('INSERT INTO categorias (nombre, descripcion) VALUES (?,?)').run(n, d);

    const adminPass = bcrypt.hashSync('admin123', 10);
    const adminPin = bcrypt.hashSync('1234', 10);
    const cajeroPass = bcrypt.hashSync('cajero123', 10);
    const ins = (n, u, p, r, s, pin) => db.prepare('INSERT INTO usuarios (nombre, usuario, password, rol, sucursal_id, pin) VALUES (?,?,?,?,?,?)').run(n, u, p, r, s, pin);
    ins('Administradora HERGON', 'admin', adminPass, 'administrador', 1, adminPin);
    ins('María López', 'maria', cajeroPass, 'cajero', 1, null);
    ins('Juan Pérez', 'juan', cajeroPass, 'cajero', 1, null);
    ins('Ana García', 'ana', cajeroPass, 'cajero', 2, null);
    ins('Pedro Sánchez', 'pedro', cajeroPass, 'cajero', 2, null);
    ins('Laura Martínez', 'laura', cajeroPass, 'cajero', 3, null);
    ins('Carlos Ruiz', 'carlos', cajeroPass, 'cajero', 3, null);
    ins('Rosa Hernández', 'rosa', cajeroPass, 'cajero', 4, null);
    ins('Diego Torres', 'diego', cajeroPass, 'cajero', 4, null);

    db.prepare('INSERT INTO tipos_membresia (nombre, descuento) VALUES (?,?)').run('Bronce', 5);
    db.prepare('INSERT INTO tipos_membresia (nombre, descuento) VALUES (?,?)').run('Plata', 10);
    db.prepare('INSERT INTO tipos_membresia (nombre, descuento) VALUES (?,?)').run('Oro', 15);
    ...
    for (let i = 0; i < productos.length; i++) {
      const p = productos[i];
      db.prepare('INSERT INTO productos (clave, nombre, descripcion, categoria_id, precio_menudeo, precio_medio_mayoreo, precio_mayoreo, cantidad_medio_mayoreo, cantidad_mayoreo, stock_minimo) VALUES (?,?,?,?,?,?,?,?,?,?)').run(...p);
      const prodId = i + 1;
      for (let s = 1; s <= 4; s++) {
        const stock = Math.floor(rand() * 200) + 20;
        db.prepare('INSERT INTO inventario (producto_id, sucursal_id, stock) VALUES (?,?,?)').run(prodId, s, stock);
      }
    }

    db.prepare('INSERT INTO clientes (nombre, telefono, membresia_id) VALUES (?,?,?)').run('Taller Mecánico El Rápido', '228-100-0001', 3);
    db.prepare('INSERT INTO clientes (nombre, telefono, membresia_id) VALUES (?,?,?)').run('Autopartes González', '228-100-0002', 2);
    db.prepare('INSERT INTO clientes (nombre, telefono, membresia_id) VALUES (?,?,?)').run('Servicio Automotriz López', '228-100-0003', 2);
    db.prepare('INSERT INTO clientes (nombre, telefono, membresia_id) VALUES (?,?,?)').run('Roberto Sánchez', '228-100-0004', 1);
    db.prepare('INSERT INTO clientes (nombre, telefono) VALUES (?,?)').run('Público General', null);

    db.prepare('INSERT INTO promociones (nombre, tipo, valor, aplica_a, categoria_id, fecha_inicio, fecha_fin) VALUES (?,?,?,?,?,?,?)').run('Descuento Filtros Abril', 'porcentaje', 10, 'categoria', 9, '2026-04-01', '2026-04-30');
  ...
  setInterval(() => { try { db.save(); } catch(e) {} }, 5000);
